...
impl AnimationManager {
    ...
    pub fn update(&mut self, model: &ModelData, delta_seconds: f32) {
        ...
        if !any_active {
            // Leave matrices_dirty alone. If the renderer hasn't yet
            // uploaded the initial bind-pose matrices (e.g. for a
            // skinned model with no active animation, like a Halo
            // rigid mesh), the constructor's `dirty = true` survives
            // into the first render call.
            return;
        }

        // We're about to recompute — clear so we can re-set on success.
        ...
        if let Some(root) = model.nodes.iter().position(|n| n.parent_index == -1) {
