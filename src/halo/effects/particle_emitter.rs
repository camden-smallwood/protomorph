...
pub fn pulse_emitter(
    ...
    // Cap to the per-frame fair-share budget BEFORE decrementing the
    // accumulator, so the capped overflow (`n - max_new`) stays accumulated
    // and spawns next frame instead of being silently dropped under budget
    // pressure (the very case `max_new` exists for).
    if n > max_new as f32 {
        n = max_new as f32;
    }
    rt.accumulator -= n;
