...
use crate::halo::effects::particle_emitter::{ParticleState, RawParticleState};
...
pub struct ParticleGpu {
    ...
    /// is recreated on resize). `draw_batch` binds it inside the shared
...
pub struct BatchMaterial {
    ...
    pub eval: crate::halo::effects::particle_curves::EmitterEvalTables,
...
impl ParticleGpu {
    pub fn new(shared: &SharedResources) -> Self {
        ...
        func_tables.unmap();
        use crate::halo::effects::particle_curves::{
    ...
    /// [`Self::draw_batch`] from inside that rpass. `depth_view` is the
    ...
    /// Draw one batch's billboards INSIDE the shared transparents rpass
    /// (F-full). Sets the batch's blend pipeline, the color group0 (built
    /// by [`Self::prepare_color_render`]), and the batch material, then
    /// draws its scattered 16-instance row spans (empty/dead slots self-
    /// cull in the VS). No-op if group0 isn't prepared or the batch isn't
    /// a drawable color batch. The caller must re-bind its own group0 after
    /// (the particle group0 layout differs from the structure layout).
    pub fn draw_batch<'rp>(
        ...
        let bi = batch_index as u32;
        rpass.set_pipeline(pipeline);
        rpass.set_bind_group(0, group0, &[]);
        rpass.set_bind_group(1, &batch.bind_group, &[]);
        // Particles of a batch live in scattered rows (engine row
        // allocation), not a contiguous region. Coalesce consecutive rows
        // of the same batch into one draw to cut call count.
        let mut row = 0u32;
        ...
        while row < rows {
            ...
            // rows [start, row) → grid slots [start*16, row*16).
            rpass.draw(0..6, start * 16..row * 16);
        }
    }
...
mod eval_tests {
    ...
    /// periodic value at time 0 is an exact grid hit (phase index 0) for the
    /// non-wrap types; verify against the raw table byte.
    #[test]
    fn periodic_time_zero_exact() {
        for ftype in 1u8..=11 {
            // skip the sawtooth-wrap family (6/7) where the seam blend applies.
            if ftype == 6 || ftype == 7 {
                continue;
            }
            let base = 7168 + (ftype as usize - 1) * 1024;
            let got = periodic_function_evaluate(ftype, 0.0);
            assert!(
                (got - tbyte(base, 0)).abs() < 1e-4,
                "periodic type {ftype} @0: {got} vs {}",
                tbyte(base, 0)
            );
        }
    }
