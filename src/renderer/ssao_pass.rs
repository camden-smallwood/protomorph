...
impl SsaoPass {
    pub fn new(shared: &SharedResources, gbuffer: &GBuffer) -> Self {
        let bgl = create_ssao_bgl(&shared.device);
        let pipeline = create_ssao_pipeline(&shared.device, &bgl);

        let mut ssao_params = generate_ssao_params();
        ...
        let noise_view = create_ssao_noise_texture(&shared.device, &shared.queue);
    ...
    pub fn resize(&mut self, shared: &SharedResources, gbuffer: &GBuffer) {
        let ssao_half_w = (shared.config.width / 2).max(1) as f32;
        let ssao_half_h = (shared.config.height / 2).max(1) as f32;
...
fn generate_ssao_params() -> SsaoParams {
...
fn create_ssao_bgl(device: &wgpu::Device) -> wgpu::BindGroupLayout {
...
fn create_ssao_pipeline(device: &wgpu::Device, ssao_bgl: &wgpu::BindGroupLayout) -> wgpu::RenderPipeline {
