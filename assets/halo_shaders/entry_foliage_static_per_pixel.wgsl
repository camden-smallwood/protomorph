// Halo `foliage / static_per_pixel` entry point.
//
// Faithful port of `foliage_fx.hlsl` static_per_pixel path
// (foliage_fx.hlsl:319-345). Per engine comment line 335:
//   "no one should be using the foliage shader and per pixel lighting,
//    output red as a warning."
//
// Implementation: identical to static_sh — calls the same VS body
// that evaluates `ravi_order_3` at the vertex normal using the
// default-lighting cbuffer — but afterward forces lighting.g and
// lighting.b to 0.0 so the foliage tints visibly RED. The intent is
// a content-author warning: an authored foliage material should be
// dispatched through one of the SH lighting variants (static_sh /
// static_per_vertex / static_prt_ambient), not per_pixel.
//
// PS body is `static_common_ps` — same as static_sh.
//
...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    // Engine `static_per_pixel_vs` (foliage_fx.hlsl:336):
    //   "no one should be using the foliage shader and per pixel
    //    lighting, output red as a warning."
    //   vsout.lighting.g = vsout.lighting.b = 0.0f;
    // Compute the same SH3 lighting as static_sh then zero G+B so any
    // surface rendered via this entry tints RED — surfaces a content
    // author for this case.
    var sh_lighting = ravi_order_3(world_normal, build_default_sh_array());
    sh_lighting.g = 0.0;
    sh_lighting.b = 0.0;
