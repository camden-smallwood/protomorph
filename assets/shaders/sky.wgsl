// Procedural atmospheric sky — fills sky pixels (where G-buffer is empty)
// Uses the same atmosphere parameters as the lighting pass fog.

// Group 0: G-buffer position_depth (to detect sky pixels) + sampler
@group(0) @binding(0) var t_position_depth: texture_2d<f32>;
@group(0) @binding(1) var s_nearest: sampler;

// Group 1: Sky params + atmosphere data
struct SkyParams {
    inverse_view_projection: mat4x4<f32>,
    camera_position: vec3<f32>,
    _pad: f32,
};
@group(1) @binding(0) var<uniform> sky: SkyParams;
...
// Sun disk parameters
const SUN_ANGULAR_RADIUS: f32 = 0.00935;  // ~0.536 degrees
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Sky pixel detection: clip_z (w channel) is 0 for cleared G-buffer, > 0 for geometry
    let pos_sample = textureSample(t_position_depth, s_nearest, uv);
    if (pos_sample.w > 0.0) {
        discard;
    }
    ...
    // Compute sky optical depth using the full atmospheric column along the ray.
    // The fog uses max_fog_thickness (short range), but sky rays traverse the entire
    // atmosphere. For a ray at elevation angle α, the air mass (path through the
    // exponential atmosphere) is height_scale / sin(α). Near the horizon sin(α)→0
    // so we clamp to avoid infinity.
    let h_cam = max(sky.camera_position.z - atmosphere.reference_height, 0.0);
    let sin_elev = max(ray_dir.z, 0.02); // clamp near-horizon to avoid infinity
    let air_mass = 1.0 / sin_elev;

    // Total column optical depth from camera height to infinity, scaled by air mass
    ...
    // Inscatter — for sky, this IS the color (no surface to attenuate).
    // The fog's compute_scattering omits solar luminance since scene lighting handles it.
    // For the sky we need it explicitly: sun luminance drives the scattering brightness.
